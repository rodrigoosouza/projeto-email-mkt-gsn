...
interface Props {
  ...
  /** Em share mode, oculta filtros se forem fixos */
...
function formatFullNumber(value: number): string { return value.toLocaleString('pt-BR') }
...
function KpiCard({ title, value, subtitle, change, icon: Icon, iconColor, invertChange = false }) {
...
export function MetaAdsView({
  ...
  const [campaignFilter, setCampaignFilter] = useState(defaultFilters?.campaignFilter || 'all')
  ...
  const [dateRange, setDateRange] = useState<DateRange>(() => {
    ...
    if (defaultFilters?.dateFrom && defaultFilters?.dateTo) {
      return { from: parseDate(defaultFilters.dateFrom), to: parseDate(defaultFilters.dateTo) }
    }
    return { from: subDays(today, 29), to: today }
  })

  const filteredByDate = useMemo(() => insights.filter((i) => {
  ...
  const previousPeriodInsights = useMemo(() => {
    ...
    if (campaignFilter !== 'all') filtered = filtered.filter((i) => i.campaign_name === campaignFilter)
    return filtered
  }, [insights, dateRange, campaignFilter])

  const campaigns = useMemo(() => Array.from(new Set(filteredByDate.map((i) => i.campaign_name))).sort(), [filteredByDate])
  ...
  const dailyData = useMemo(() => {
    ...
    filteredInsights.forEach((i) => {
      const e = map.get(i.date) || { date: i.date, spend: 0, leads: 0, clicks: 0, impressions: 0, cpl: 0 }
      e.spend += Number(i.spend); e.leads += Number(i.leads); e.clicks += Number(i.clicks); e.impressions += Number(i.impressions)
      map.set(i.date, e)
    })
    const arr = Array.from(map.values()).sort((a, b) => a.date.localeCompare(b.date))
    arr.forEach((d) => { d.cpl = d.leads > 0 ? d.spend / d.leads : 0 })
    return arr.map((d) => ({ ...d, label: format(parseDate(d.date), 'dd/MM', { locale: ptBR }) }))
  }, [filteredInsights])

  const campaignSummary = useMemo(() => {
    const map = new Map<string, any>()
    filteredInsights.forEach((i) => {
      const e = map.get(i.campaign_name) || {
        campaign_name: i.campaign_name,
        spend: 0, impressions: 0, clicks: 0, reach: 0, leads: 0, days: 0, ctr: 0, cpl: 0,
      }
      e.spend += Number(i.spend); e.impressions += Number(i.impressions); e.clicks += Number(i.clicks)
      e.reach += Number(i.reach); e.leads += Number(i.leads); e.days += 1
      map.set(i.campaign_name, e)
    })
  ...
  const maxCampaignSpend = useMemo(() => Math.max(...campaignSummary.map((c) => c.spend), 1), [campaignSummary])
  ...
  const chartConfig = {
    spend: { label: 'Investimento', color: '#3b82f6' },
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {orgName} — Performance das campanhas
            </p>
          </div>
        </div>
        {mode === 'auth' && (
      ...
      {/* Filtros — escondidos se filtersLocked */}
      {!filtersLocked && (
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-5">
        <KpiCard title="Investimento" value={formatCurrency(kpis.totalSpend)} subtitle={`CPC: ${formatCurrency(kpis.avgCPC)}`}
          change={calcChange(kpis.totalSpend, prevKpis.totalSpend)} icon={DollarSign} iconColor="bg-amber-500" invertChange />
        <KpiCard title="Leads" value={formatFullNumber(kpis.totalLeads)} subtitle={`CPL: ${formatCurrency(kpis.avgCPL)}`}
          change={calcChange(kpis.totalLeads, prevKpis.totalLeads)} icon={Users} iconColor="bg-emerald-500" />
        <KpiCard title="Taxa de Conversao"
          value={kpis.totalClicks > 0 ? formatPercent((kpis.totalLeads / kpis.totalClicks) * 100) : '0%'}
          subtitle="Leads / Cliques" change={null} icon={TrendingUp} iconColor="bg-teal-500" />
        <KpiCard title="Impressões" value={formatNumber(kpis.totalImpressions)} subtitle={`Alcance: ${formatNumber(kpis.totalReach)}`}
          change={calcChange(kpis.totalImpressions, prevKpis.totalImpressions)} icon={Eye} iconColor="bg-blue-500" />
        <KpiCard title="Cliques" value={formatFullNumber(kpis.totalClicks)} subtitle={`CTR: ${formatPercent(kpis.avgCTR)}`}
          change={calcChange(kpis.totalClicks, prevKpis.totalClicks)} icon={MousePointerClick} iconColor="bg-violet-500" />
      </div>
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{chartConfig[chartMetric].label} por dia no período</CardDescription>
            </div>
            <div className="flex gap-1">
              {(Object.keys(chartConfig) as Array<keyof typeof chartConfig>).map((key) => (
                <Button key={key} variant={chartMetric === key ? 'default' : 'ghost'} size="sm" className="text-xs h-7 px-3" onClick={() => setChartMetric(key)}>
            ...
            <div className="h-[320px] mt-2">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dailyData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <XAxis dataKey="label" tick={{ fontSize: 11 }} tickLine={false} axisLine={false} dy={8} />
                  <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false}
                    tickFormatter={(v) => chartMetric === 'spend' || chartMetric === 'cpl' ? formatCompactCurrency(v) : formatNumber(v)}
                    width={65} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey={chartMetric} name={chartConfig[chartMetric].label}
                    stroke={chartConfig[chartMetric].color} strokeWidth={2} fill="url(#colorMetric)"
                    dot={{ r: 3, strokeWidth: 2, fill: 'white' }} activeDot={{ r: 5, strokeWidth: 2 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>{campaignSummary.length} campanha{campaignSummary.length !== 1 ? 's' : ''} no período</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right w-[80px]"><span className="text-xs">Conv.</span></TableHead>
                    <TableHead className="text-right w-[80px]">
                ...
                <TableBody>
                    ...
                    <TableRow key={c.campaign_name} className="group">
                      <TableCell>
                        <div className="space-y-1.5">
                          <span className="font-medium text-sm block break-words" title={c.campaign_name}>{c.campaign_name}</span>
                      ...
                      <TableCell className="text-right text-muted-foreground">{formatNumber(c.impressions)}</TableCell>
                      <TableCell className="text-right text-muted-foreground">{formatFullNumber(c.clicks)}</TableCell>
                      <TableCell className="text-right">
                        <span className={cn('text-xs font-medium px-1.5 py-0.5 rounded',
                          c.ctr >= 2 ? 'bg-emerald-500/10 text-emerald-600' :
                          c.ctr >= 1 ? 'bg-amber-500/10 text-amber-600' : 'bg-red-500/10 text-red-600')}>
                          {formatPercent(c.ctr)}
                        </span>
                      </TableCell>
                      <TableCell className="text-right">
                        {c.clicks > 0 && c.leads > 0 ? (
                          <span className={cn('text-xs font-medium px-1.5 py-0.5 rounded',
                            ...
                            (c.leads / c.clicks) * 100 >= 2 ? 'bg-amber-500/10 text-amber-600' : 'bg-red-500/10 text-red-600')}>
                            {formatPercent((c.leads / c.clicks) * 100)}
                          </span>
                        ) : <span className="text-muted-foreground">—</span>}
                      </TableCell>
                      <TableCell className="text-right">
                        {c.leads > 0 ? <span className="font-semibold text-emerald-600 dark:text-emerald-400">{c.leads}</span>
                          : <span className="text-muted-foreground">—</span>}
                      </TableCell>
                      <TableCell className="text-right">
                        {c.cpl > 0 ? (
                          <span className={cn('text-sm font-medium',
                            c.cpl <= 50 ? 'text-emerald-600' : c.cpl <= 80 ? 'text-amber-600' : 'text-red-600')}>
                  ...
                  <TableRow className="bg-muted/50 font-semibold border-t-2">
                    <TableCell className="text-sm">TOTAL ({campaignSummary.length} campanhas)</TableCell>
                    ...
                    <TableCell className="text-right">
                      <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-muted">{formatPercent(kpis.avgCTR)}</span>
                    </TableCell>
                    <TableCell className="text-right">
                    ...
                    <TableCell className="text-right text-emerald-600 dark:text-emerald-400">{kpis.totalLeads}</TableCell>
                    <TableCell className="text-right">{kpis.avgCPL > 0 ? formatCurrency(kpis.avgCPL) : '—'}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </div>
      ...
      {campaignFilter !== 'all' && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>{campaignFilter.length > 60 ? campaignFilter.substring(0, 60) + '...' : campaignFilter}</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredInsights.sort((a, b) => b.date.localeCompare(a.date)).map((i) => (
                  <TableRow key={i.id}>
                    <TableCell className="font-medium">{format(parseDate(i.date), 'dd/MM/yyyy', { locale: ptBR })}</TableCell>
                    ...
                    <TableCell className="text-right">
                      {Number(i.leads) > 0 ? <span className="font-semibold text-emerald-600 dark:text-emerald-400">{i.leads}</span>
                        : <span className="text-muted-foreground">—</span>}
                    </TableCell>
                    <TableCell className="text-right">
